...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h3 mb-1"><?= htmlspecialchars((string) $job['name'], ENT_QUOTES, 'UTF-8') ?></h1>
        <p class="text-body-secondary mb-0">Echte Transfers schreiben in die konfigurierte Target Connection.</p>
    </div>
    <a class="btn btn-outline-secondary" href="/admin/jobs">Zurück</a>
</div>
...
<div class="d-flex gap-2 mb-4 flex-wrap">
    <form method="post" action="/admin/jobs/<?= (int) $job['id'] ?>/dry-run"><button class="btn btn-success">Dry Run starten</button></form>
    <form method="post" action="/admin/jobs/<?= (int) $job['id'] ?>/run"><input type="hidden" name="confirm" value="run"><button class="btn btn-danger">Echten Transfer starten</button></form>
    <a class="btn btn-outline-primary" href="/admin/jobs/<?= (int) $job['id'] ?>/runs">Runs anzeigen</a>
    <form method="post" action="/admin/jobs/<?= (int) $job['id'] ?>/delete" onsubmit="return confirm('Diesen Job inklusive Runs und Logs löschen?');">
        ...
        <button class="btn btn-outline-danger" type="submit">Job löschen</button>
    </form>
</div>
